...
serpent_block_t serpent_encrypt (serpent_block_t plaintext, serpent_key_t key, serpent_block_t maskIn, serpent_block_t maskOut) {
...
serpent_block_t remaskOutput(serpent_block_t ciphertext, serpent_block_t maskIn, serpent_block_t maskOut){
	...
	serpent_block_t remask = maskOut ^ invLinearTransformation(maskIn);
	return ciphertext ^ remask;

}

void reconfigure(serpent_block_t maskIn, serpent_block_t maskOut){

	serpent_block_t mask1[2] = {initialPermutation(maskIn), initialPermutation(maskOut)};
	serpent_block_t mask2Inv[2] = {initialPermutation(invLinearTransformation(maskOut)), initialPermutation(invLinearTransformation(maskIn))};
	...
	#pragma HLS unroll
		...
		#pragma HLS unroll
			...
			#pragma HLS unroll
				...
				serpent_sbox_t idx = mask1[i%2](4*k + 3, 4*k) ^ (serpent_sbox_t) j;
				serpent_sbox_t val = sBoxClean[i][j] ^ mask2Inv[i%2](4*k + 3, 4*k);
...
serpent_roundkey_t getRoundKey(serpent_prekey_t prekey, serpent_round_idx_t i){
	...
	roundKey = substitutionLayerKey(prekey(127,0), k);
...
serpent_block_t substitutionLayer(serpent_block_t block, serpent_sbox_idx_t idx){
	...
	#pragma HLS unroll
		...
		substituedBlock(i*4 + 3, i*4) = sBoxMasked[idx][i][permutedBlock(i*4 + 3, i*4)];
...
serpent_block_t substitutionLayerKey(serpent_block_t block, serpent_sbox_idx_t idx){
	...
	#pragma HLS unroll
		...
		substituedBlock(i*4 + 3, i*4) = sBoxClean[idx][permutedBlock(i*4 + 3, i*4)];
...
serpent_block_t invLinearTransformation(serpent_block_t ctx){
